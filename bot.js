...
if (err) console.error(err);
    ...
    client.on('message', message => {
        ...
        if(!profile[message.author.id].tite) profile[message.author.id].tite = "Gold-Bot"
        if(!tit) {
...
if (!profile[getvalueof.id]) profile[getvalueof.id] = {points: 0,reps: "NOT YET",credits: 1, level: 1,tite: "Gold-Bot", rep: 0, lastDaily: "NOT COLLECTED"};
            let Image = Canvas.Image,
